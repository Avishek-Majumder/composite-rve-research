...
**M9 implementation status:** IN PROGRESS — Steps 1-8 design/QC gates complete; Step 9 in progress — protocol/case/identity/validation-seed and implementation-boundary topology locks complete; src/28 defective-void generator, src/29 defective mesher, and src/30 defective PBC solver created, independently static/lineage-audited, and durably Git-closed; src/31 transfer-case orchestrator now also created, independently audited, and durably Git-closed; Step-9 scientific execution not yet started
...
| M9        | IN PROGRESS   | Final Parameter-Space Lock and Stochastic Pilot Dataset — Steps 1-8 design/QC gates complete; Step 9 in progress — protocol/topology locked; src/28-31 durably closed; Step-9 scientific execution not yet started |
...
- **M9 Step 9:** IN PROGRESS — targeted six-case transfer matrix / physical-case identity / validation-seed protocol / implementation topology / non-production evidence root locked; src/28 defective-void generator, src/29 defective mesher, src/30 defective PBC solver, and `src/31_run_m9_step9_transfer_case.py` are now created, independently audited, committed, pushed, and durably Git-closed. Durable src31 closure authority is commit `0484520f3f589f0d0c055f72eec03ee6cc97a342` with source SHA-256 `c708200df0d86a1d4ff909e40a2090b2740d9993e35fe6a1f73947fbbf19ca29`; Step-9 scientific execution has not started.
...
The current active scientific gate is:
...
M9 Step 7 has now locked the stochastic reproducibility authority.

The permanent design-identity namespace is:

`composite-rve-m9-design-v1`.

The permanent stochastic seed namespace is:

`composite-rve-m9-stochastic-v1`.

The complete six-input canonical design material is hashed with SHA-256.

The full 256-bit design digest remains authoritative; the compact human
design ID uses the first 128 digest bits.

Realization indices are positive integers beginning at 1 and are rendered as
decimal text left-zero-padded to a minimum width of six.

Every realization identity is bound to its design identity.

Particle and void seeds are domain-separated deterministic SHA-256-derived
128-bit integer encodings.

The explicit RNG construction is:

`Generator(PCG64(seed))`.

For pristine cases, a void-placement seed is `not_applicable`; a fabricated
numerical void seed is not introduced.

Seed integers are provenance, not ordinary ML predictors.

Seed alone is not complete exact-geometry replay authority.

Replay provenance must also bind design identity, realization identity,
generator/source authority, RNG implementation, environment/build authority,
and geometry identity.

Successful durable scientific evidence must not be overwritten.

Failed durable scientific evidence must not be overwritten.
...
An authorized retry preserves the same scientific realization identity and
