...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/02_single_particle.yaml"),
        help=(
            "Path to the YAML configuration file. "
            "Defaults to configs/02_single_particle.yaml."
        ),
    )
    parser.add_argument(
        "--mesh-size",
    ...
    model_dimension = int(config["model"]["dimension"])
    model_assumption = str(config["model"]["assumption"])
    interface_assumption = str(config["model"]["interface"])

    if model_dimension != 2:
        raise ValueError(
            "This verified solver currently supports only model.dimension = 2."
        )

    if model_assumption != "plane_stress":
        raise ValueError(
            "This verified solver currently supports only "
    ...
    def validate_isotropic_material(name, E, nu):
        """Validate isotropic linear-elastic material parameters."""
        if not np.isfinite(E) or E <= 0.0:
            raise ValueError(
                f"{name}.youngs_modulus must be finite and positive."
            )

        if not np.isfinite(nu) or not (-1.0 < nu < 0.5):
            raise ValueError(
                f"{name}.poissons_ratio must satisfy -1 < nu < 0.5."
            )
    ...
    if not particle_strictly_inside_rve:
        raise ValueError(
            "The circular particle must lie strictly inside the rectangular "
            "RVE: center_x +/- radius must remain within (0, width), and "
            "center_y +/- radius must remain within (0, height)."
        )
